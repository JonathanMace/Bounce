...
import im.mace.android.bounce.objects.Bucket;
import im.mace.android.bounce.objects.GameBorder;
import im.mace.android.bounce.objects.TargetingLine;
...
public abstract class GameScene extends Scene {
    ...
    public GameScene(LevelDef def, TextureBucket textures) {
        this.def = def;
        ...
        this.world.setContactListener(new GoalCheckHandler());
    ...
    private void fire(Vector2 velocity) {
        if (this.ball!=null) {
            Body ballBody = this.world.getPhysicsConnectorManager().findBodyByShape(this.ball);
            if (ballBody!=null) {
    ...
    // Called whenever the ball bounces, checks goal condition
    private class GoalCheckHandler implements ContactListener {
        public void beginContact(Contact contact) {
            if (!GameScene.this.levelComplete && GameScene.this.goalAchieved()) {
                GameScene.this.levelComplete = true;
            }
        }
        public void endContact(Contact contact) { /* Do nothing */ }
